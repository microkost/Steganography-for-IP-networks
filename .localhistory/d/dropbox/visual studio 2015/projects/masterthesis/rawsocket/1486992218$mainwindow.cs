...
namespace RawSocket
{
    public partial class MainWindow : Form
    {
        ...
        private void Listening(bool isServerListening, string tmpIp)
        {
            ...
            selectedDevice = allDevices[getSelectedInterfaceIndex(tmpIp)]; // Take the selected adapter
        ...
        private void buttonSteganogr_Click(object sender, EventArgs e)
        {
            ...
            { 
                ...
                {
                    Source = new IpV4Address(GetSeletedIP(false)),
                    CurrentDestination = new IpV4Address(GetSeletedIP(true)),
                    Ttl = 128,
                };
        ...
        private int getSelectedInterfaceIndex(string tmpIp)
        {
            //open device section
        ...
        private string GetSeletedIP(bool isServer)
        {
            string tmpIp;
            if (isServer)
            {
                tmpIp = comboBoxServerAddress.SelectedValue.ToString();
                //Additional information: Operace mezi vlákny není platná: Přístup k ovládacímu prvku comboBoxServerAddress proběhl z jiného vlákna než z vlákna, v rámci kterého byl vytvořen.
            }
            else
            {
                tmpIp = comboBoxClientAddress.SelectedValue.ToString();
