...
namespace RawSocket
{
    public partial class MainWindow : Form
    {
        ...
        private void InitialProcedure()
        {
            ...
            comboBoxClientAddress.SelectedIndex = 8; //hardcoded
            //add localhost
                ...
                case ('0'): //FTP
                    {
                        if(checkBoxClient.Checked)
                        {
                            //https://libftp.codeplex.com/

                            List<ftpinfo> files = testftp.connect(comboBoxServerAddress.Text, "admin", "admin"); //(txtHost.Text, txtUsername.Text, txtPassword.Text)
                        }
                        else
                        {
                            //server is checked
